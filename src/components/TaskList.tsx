...
import { CheckCircle, Circle, Trash2, Edit, Save, X, Clock } from 'lucide-react';
...
export default function TaskList() {
  ...
  }, [authLoading, currentUser?.id, fetchTasks]);
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    const isoDueDate = newTaskDueDate ? new Date(newTaskDueDate + 'T00:00:00Z').toISOString() : null;
    ...
    const taskData = {
      ...
      categories: newTaskCategories.trim() ? newTaskCategories.split(',').map(tag => tag.trim()).filter(tag => tag) : null,
    };

    const addPromise = addTask(taskData, currentUser.id);
    toast.promise(addPromise, {
      loading: 'Adding task...',
      success: 'Task added successfully!',
      error: (err) => `Failed to add task: ${err.message || 'An error occurred'}`,
    });
  ...
  const handleToggleComplete = async (taskId: string, isCompleted: boolean) => {
    ...
    toast.promise(updatePromise, {
      ...
      success: isCompleted ? 'Task marked incomplete!' : 'Task marked complete!',
      error: (err) => `Failed to update task: ${err.message || 'An error occurred'}`,
    });
  };
  ...
  const handleSaveEdit = async (taskId: string) => {
    ...
    const isoDueDate = editingDueDate ? new Date(editingDueDate + 'T00:00:00Z').toISOString() : null;
    ...
    const updatedData = {
      ...
      categories: editingCategories.trim() ? editingCategories.split(',').map(tag => tag.trim()).filter(tag => tag) : null,
    };
    ...
    toast.promise(updatePromise, {
      ...
      error: (err) => `Failed to save changes: ${err.message || 'An error occurred'}`,
    });
  ...
  const handleDeleteTask = async (taskId: string) => {
    ...
    toast.promise(deletePromise, {
      ...
      error: (err) => `Failed to delete task: ${err.message || 'An error occurred'}`,
    });
  };
  ...
  if (!currentUser) {
    return (
      <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md text-center">
        <p className="text-red-500 dark:text-red-400">Please log in to manage your tasks.</p>
      </div>
    );
  }
