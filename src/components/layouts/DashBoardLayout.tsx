...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const handleLogout = async () => {
    setLoadingUser(true);
    const logoutPromise = supabase.auth.signOut(); // Store the promise

    // Use toast.promise to show loading, success, and error states automatically
    await toast.promise(logoutPromise, {
      loading: "Logging out...",
      success: "Logged out successfully!",
      error: (err) => `Logout failed: ${err.message || "An error occurred"}`,
    });

    const { error } = await logoutPromise; // Await the promise to get the error if any

    if (error) {
      console.error("Error logging out:", error.message);
      // toast.error(`Logout failed: ${error.message}`); // Already handled by toast.promise
      setLoadingUser(false);
    } else {
      console.log("User logged out successfully!");
      // toast.success('Logged out successfully!'); // Already handled by toast.promise
      // The AuthListener in app/layout.tsx will catch the SIGNED_OUT event
      // and update the Zustand store, triggering the redirect via useEffect above.
    }
  };
